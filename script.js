...
if (currentCityKey === cityDataKey) {
    const currentStar = document.getElementById('currentCityStar');
    if (currentStar) {
...
const currentStar = 
document.getElementById('currentCityStar');
if (currentStar) {
    if (favoriteCities.some(c => c.dataKey === currentCityKey)) {
        currentStar.textContent = '☆';
...
if (currentStar) {
    ...
    currentStar.addEventListener('click', (e) => {
        ...
        if(!currentCityKey)
            return;
        if (favoriteCities.some(c => c.dataKey === currentCityKey)) {
            removeFromFavorites(currentCityKey);
        } else {
            const cityName = 
            document.getElementById('cityName').textContent;

            addToFavorites(cityName,currentCityKey);
        }
    });
}
});
